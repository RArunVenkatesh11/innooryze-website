...
integrations.js is the spam/analytics seam. ANALYTICS_ID alone installs nothing. Add a consent-aware adapter, public identifiers and approved policy text as required; keep credentials server-side. Update CSP allowlists deliberately when adding external scripts/requests. Local policy entries in src/content/policies.mjs publish only after approved content is supplied; external approved policy URLs are also supported.
