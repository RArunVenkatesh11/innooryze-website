...
import {leadryzeCrm,site,services,articles,work,platforms} from '../src/site.mjs';
...
add('/products','AI Products & Custom SaaS Development','LeadRyze AI, LeadRyze CRM, IMMA and the custom SaaS, business applications and AI-enabled products InnooRyze designs and builds for clients.',inner.productsPage(),{crumb:'Products'});
...
add('/products/imma','IMMA — Marketing Maturity Assessment','IMMA is a live marketing maturity assessment from InnooRyze. Get an overall score, maturity level, category-wise results, recommendations and a roadmap.',inner.immaPage(),{crumb:'IMMA',schema:[{'@type':'WebApplication','@id':site.url+'/products/imma#software',name:'IMMA',alternateName:'Intelligent Marketing Maturity Assessment',url:site.url+'/products/imma',description:'IMMA is a live marketing maturity assessment from InnooRyze. Get an overall score, maturity level, category-wise results, recommendations and a roadmap.',applicationCategory:'BusinessApplication',featureList:['Overall score','Maturity level','Category-wise results','Recommendations','Roadmap'],publisher:{'@id':`${site.url}/#organization`}}]});
...
{const NL=String.fromCharCode(10);const idx=pages.filter(p=>p.indexable!==false);
 const section=(label,test,extra=[])=>{const list=idx.filter(p=>test(p.path));return list.length?['## '+label,'',...list.map(p=>'- ['+p.title.replace(' | InnooRyze','')+']('+new URL(p.path,site.url).href+'): '+p.description),...extra,'']:[];};
 // LeadRyze CRM has no route of its own; it is described on /products, where this anchor points.
 const crm='- ['+leadryzeCrm.name+']('+new URL(leadryzeCrm.anchor,site.url).href+'): '+leadryzeCrm.label+'. '+leadryzeCrm.description+' '+leadryzeCrm.distinction;
 fs.writeFileSync(path.join(out,'llms.txt'),['# '+site.name,'','> '+site.description,'',
  ...
  ...section('Products',p=>p.startsWith('/products'),[crm]),...section('Client work and case studies',p=>p.startsWith('/work')),
